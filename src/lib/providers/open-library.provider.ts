...
export class OpenLibraryProvider implements BookProvider {
  ...
  private readonly userAgent = 'EntrePaginas/1.0 (karole-reader; contact: local)';
  ...
  async searchBooks(query: string, filters?: SearchFilters): Promise<BookSearchResult> {
    try {
      ...
      url.searchParams.set('limit', String(filters?.limit || 15));
      ...
      if (filters?.genre) url.searchParams.set('subject', filters.genre);
      if (filters?.language) {
        // Mapeia pt para por se necessário
        const langCode = filters.language === 'pt' ? 'por' : filters.language;
      ...
      console.error('[OpenLibraryProvider] Erro ao buscar livros:', error);
      return { totalCount: 0, items: [], page: filters?.page || 1, provider: this.name };
    }
  }
  ...
  async getBook(id: string): Promise<UnifiedBook | null> {
    try {
      ...
      const res = await fetch(`${this.baseUrl}${workKey}.json`, {
        headers: { 'User-Agent': this.userAgent },
        next: { revalidate: 3600 },
      });
      ...
      return {
        ...
        description: description || 'Informações bibliográficas catalogadas pela Open Library.',
        coverUrl,
        language: 'pt',
        genres: genres.length > 0 ? genres : ['Literatura'],
        isPublicDomain: false,
        license: 'Informações do livro disponíveis — download não disponível nesta fonte.',
        officialSourceUrl: `${this.baseUrl}${work.key}`,
        sources: [
          {
            sourceName: 'openlibrary',
            externalId: work.key,
            canonicalUrl: `${this.baseUrl}${work.key}`,
      ...
      console.error(`[OpenLibraryProvider] Erro ao buscar obra ${id}:`, error);
      return null;
    }
  }

  async getDownloadOptions(_id: string): Promise<DownloadOption[]> {
    // Open Library opera primordialmente como catálogo de metadados / discovery legal
    return [];
  }
  ...
  private mapDocToUnifiedBook(doc: OpenLibraryDoc): UnifiedBook {
    ...
    return {
      ...
      officialSourceUrl: `${this.baseUrl}${doc.key}`,
      sources: [
        {
          sourceName: 'openlibrary',
          externalId: workId,
          canonicalUrl: `${this.baseUrl}${doc.key}`,
